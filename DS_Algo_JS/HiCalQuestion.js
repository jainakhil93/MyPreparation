function mergeRanges(meetings){
  ...
  for(let i=1; i<sortedMeetings.length; i++){
    const currentMeet = sortedMeetings[i];
    const lastMeetMerged = mergedMeeting[mergedMeeting.length - 1];
    if(currentMeet.startTime <= lastMeetMerged.endTime){
      lastMeetMerged.endTime = Math.max(currentMeet.endTime, lastMeetMerged.endTime);
    } else {

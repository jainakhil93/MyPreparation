...
function createFunction() {
  ...
  // CHALLENGE 2
  // Create a function createFunctionPrinter that accepts one input and returns a function. When that created function is called,
  ...
  // /*** Uncomment these to check your work! ***/
  willCounter(); // should log 1
  willCounter(); // should log 2
  willCounter(); // should log 3
  
  jasCounter(); // should log 1
  willCounter(); // should log 4
  
  
function addByX(x) {
    const addBySomething = input => {
        return x+input
    }
    return addBySomething
}
...
return innerFunc
  ...
  // it should log the first name to the console. The second time it is invoked, it should log the second name to the console, and so on, 
  ...
  // saveOutput will then return a function that behaves exactly like the passed-in function, except for when the password string is passed 
  // in as an argument. When this happens, the returned function will return an object with all previously passed-in arguments as keys, 
  // and the corresponding outputs as values.
  function saveOutput(func, magicWord) {
    // no magic word then behave as the function passed
    // else convert into object with keys as the passed-in argument and value as the outputs
    let newObject = {}
    return value => {
      if(typeof value === 'string'){
        return newObject
      } else {
        newObject[value] = func(value)
        return func(value)
      }
    }
  }
